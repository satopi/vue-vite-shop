<template>
  <section id="merchant" class="flex-column">
    ...
    <el-card shadow="hover" class="flex-1 flex-column table-card">
      ...
      >
        ...
        <el-table-column prop="date" label="创建日期" width="150" align="center" />
  ...
  <MerchantDialog ref="merchantAddOrEdit" @success="confirmAddOrEdit" />
</template>

<script setup lang="ts">
  import FilterBar from '@/components/FilterBar/index.vue';
  import MerchantDialog from './AddOrEdit/index.vue';
  ...
  const editRow = (row: IMerchant) => {
    console.log(row);
    merchantAddOrEdit.value.show(row);
  };
  ...
  const deleteRow = (index: number) => {
    ElMessageBox.confirm('此操作将永久删除该商家, 是否继续?', '提示', {
      ...
      merchantPaging.value.pageData.splice(index, 1);
